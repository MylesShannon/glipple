...
require_once('/var/www/ampjuke/db.php');
...
require_once('/var/www/ampjuke/translate.php'); 
require_once('/var/www/ampjuke/disp.php');
require_once('/var/www/ampjuke/sql.php');
require_once('/var/www/ampjuke/sql.php');
require_once('/var/www/ampjuke/configuration.php'); 
require_once('/var/www/ampjuke/set_td_colors.php');
require_once('/var/www/ampjuke/tbl_header.php');
require_once('/var/www/ampjuke/echonest_lib.php');

// ***** CRON-SETTINGS:
// Want to run this using cron ?
// If yes, read on and uncomment+modify entries below:

$_POST['details']='3'; // 1-4 -> the higher the more will be displayed/printed.
...
$cron='1';
error_reporting(0);
// ***** CRON-SETTINGS ends. -> Nothing more to change/alter if running this in CRON - just hook up your job and run it frequently!
...
function loc_print_parameter_status($total_tracks,$nr,$name,$checkbox_name) {
	$ret='<td>...'.$name.':</td><td><b>'.($total_tracks-$nr).'</b> ('.(100-round(($nr/$total_tracks)*100,2)).'%) ';
	$ret.=' '.add_checkbox($checkbox_name).'</td>';
	$ret.="</tr> \n\n";
    return $ret;
}
...
function loc_get_api_ratelimit($details,$upload) {
    ...
    if ($upload=='1') { // ...if set: get the ratelimit the fancy way (by asking Echonest):
        $filename='/var/www/ampjuke/tmp/api.txt'; // *************** IMPORTANT: Change this if you f.ex. want to point to an absolute location
        loc_update_status(4,$details,'Temporary filename for API rate limit details: '.$filename);
        include('/var/www/ampjuke/db.php');
...
if ($act=='scan') {
    ...
    loc_update_status(2,$_POST['details'],'Initial query: '.$qry);
	...
	while ($row=mysql_fetch_array($result)) {
		...
		$ok=echonest_lookup_track($row['id']); // Try LAZY approach first: See echonest_lib.php
		...
		if ($ok==0) {
			...
			if ($upload=='1') {
				...
				if (!file_exists('./tmp/'.$id.'.mp3')) {
					copy($track['path'],'/var/www/ampjuke/tmp/'.$id.'.mp3');
