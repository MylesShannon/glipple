...
define('GETID3_DB_HOST',  'glipple.com');
define('GETID3_DB_USER',  'root');
define('GETID3_DB_PASS',  'dJc001Nfr35h');
...
//$getid3PHP_filename = realpath('/var/www/html/login/application/views/upload/getid3.php');
...
if (!empty($_REQUEST['scan']) || !empty($newscan) || !empty($_REQUEST['rescanerrors'])) {
	...
	if (!empty($_REQUEST['rescanerrors'])) {
	...
	} elseif (!empty($_REQUEST['scan']) || !empty($newscan)) {

		//echo '<a href="'.htmlentities($_SERVER['PHP_SELF']).'">abort</a><hr>';

		echo 'Scanning all media files in <b>'.str_replace('\\', '/', realpath(!empty($_REQUEST['scan']) ? $_REQUEST['scan'] : $newscan)).'</b> (and subdirectories)<hr>';
		...
		$DirectoriesToScan  = array(!empty($_REQUEST['scan']) ? $_REQUEST['scan'] : $newscan);
	...
	foreach ($FilesInDir as $filename) {
		...
		echo '<br> ['.number_format(++$rowcounter).' / '.number_format($totaltoprocess).'] '.str_replace('\\', '/', $filename);
		...
		if (empty($ThisFileInfo['fileformat'])) {
			...
			if (!empty($_REQUEST['rescanerrors'])) {
				...
				$SQLquery .= '"'.mysql_real_escape_string(!empty($userID) ? $userID :  999).'")';
				//echo "MYSQL: ".$userID;
				
				//$SQLquery  = 'UPDATE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` SET `filename` = `'.$musicDir."/".mysql_insert_id().'` WHERE `files`.`ID` = '.mysql_insert_id().';';
			...
			//$SQLquery  = 'UPDATE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` SET `filename` = `'.$musicDir."/".mysql_insert_id().'` WHERE `files`.`ID` = '.mysql_insert_id().';';
			//$SQLquery  = "UPDATE `getid3`.`files` SET `filename` = '".$musicDir."/".mysql_insert_id()."' WHERE `files`.`ID` = ".mysql_insert_id().";";
			
			// Update `filename` column to reflect file directory of song after being renamed to the row ID
			$lastRow = mysql_insert_id();
			$SQLquery  = 'UPDATE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` SET ';
			$SQLquery .= '  `filename` = "'.$musicDir."/".$lastRow.'.'.$ThisFileInfo['fileformat'].'"';
			$SQLquery .= 'WHERE (`ID` = "'.$lastRow.'")';
			mysql_query_safe($SQLquery);
		}
...
// echo 'Directory: <input type="text" name="newscan" size="50" value="'.htmlentities(!empty($newscan) ? $newscan : '', ENT_QUOTES).'"> ';
