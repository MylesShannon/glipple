...
function mysql_image($imageFileType){
	...
	if(mysql_num_rows(mysql_query("SELECT user_id FROM $table WHERE user_id = '$userID'")) && $row['band_image'] == ''){
	...
	} elseif(mysql_num_rows(mysql_query("SELECT user_id FROM $table WHERE user_id = '$userID'")) && $row['band_image'] != ''){
